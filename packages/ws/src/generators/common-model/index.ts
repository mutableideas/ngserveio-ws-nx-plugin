...
export default async function commonModelGenerator(
  tree: Tree,
  schema: ICommonModelSchema
) {
  ...
  const filesExist =
    tree.exists(
      `${commonProjectSrcPath}/models/${modelNames.fileName}.model.ts`
    ) &&
    tree.exists(
      `${commonProjectSrcPath}/validators/${modelNames.fileName}.validator.ts`
    );
  ...
  const commonProjectModelsBarrelPath = createBarrelFile(
    tree,
    join(commonProjectSrcPath, 'models')
  );
  const commonProjectValidatorPath = createBarrelFile(
    tree,
    join(commonProjectSrcPath, 'validators')
  );
  ...
  await generateFiles(
    ...
    commonProjectSrcPath,
    {
      ...
      template: '',
      formControls:
        schema.inputs?.trim()?.length > 0 ? parseControls(schema.inputs) : [],
    }
  );
  ...
  const fileUpdates: FileUpdates = {
    ...
    [commonProjectBarrelPath]: (sourceFile: SourceFile) => {
      ...
      const hasSpecifiers = moduleSpecifiers.some((p) =>
        containsExport(sourceFile, p)
      );
