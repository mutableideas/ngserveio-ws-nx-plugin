 ...
 */
export type NgModuleDecoratorImport = Record<
  string,
  (NgModuleImport | string)[]
>;
...
export const addImportDeclaration = (
  modulePath: string,
  imports: { importPath: string; modules: string[] }
): FileUpdates => {
  return {
...
export const containsExport = (
  sourceFile: SourceFile,
  exportText: string
): boolean => {
  return sourceFile
    .getDescendantsOfKind(ts.SyntaxKind.ExportDeclaration)
    .some((p) => {
      return p.getText().includes(exportText);
    });
};

export const getOrCreateImportDeclaration = (
  sourceFile: SourceFile,
  relativeImport: string
): ImportDeclaration => {
  return (
    sourceFile.getImportDeclaration(relativeImport) ||
    sourceFile.addImportDeclaration({
...
export const createImportClassDeclaration = (
  sourceFile: SourceFile,
  relativeImport: string,
  names: string[]
): void => {
  const importDeclaration = getOrCreateImportDeclaration(
    sourceFile,
    relativeImport
  );
  const namedImports = importDeclaration
    .getNamedImports()
    .map((v) => v.getText());
  const addedImports = names
    .filter((name) => namedImports.indexOf(name) === -1)
    .map((name) => ({ name }));
...
export class ModuleGeneratorUtil {
  public static findModuleClass(
    ...
    return sourceFile
      ...
      .find((p) => p.getDecorator(decorator));
  }

  public static addToModuleDecorator(
    sourceFile: SourceFile,
    decoratorProperties: NgModuleDecoratorProperties,
    moduleDecorator: 'NgModule' | 'Module' = 'NgModule'
  ): void {
    // Find The Angular Decorator
    const decorator = sourceFile
      .getDescendantsOfKind(ts.SyntaxKind.Decorator)
      ?.find((p) => p.getName() === moduleDecorator);
    ...
    Object.keys(decoratorProperties).forEach((ngModulePropertyName) => {
      ...
      const propertyAssignment =
        propertyMap[ngModulePropertyName] ||
      ...
      const arrayLiteral = propertyAssignment.getFirstDescendantByKind(
        ts.SyntaxKind.ArrayLiteralExpression
      );

      // import the relative file path and add to the array literal
      Object.keys(decoratorProperties[ngModulePropertyName]).forEach(
        (relativeImport) => {
          const elements =
            decoratorProperties[ngModulePropertyName][relativeImport];
          const currentElements = arrayLiteral
            .getElements()
            .map((t) => t.getText());
          const namedImports = elements.map((el) => {
            return typeof el === 'string' ? el : el.moduleName;
          });
          const arrayLiteralElements = elements
            .map((el) => {
